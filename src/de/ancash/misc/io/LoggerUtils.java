...
public class LoggerUtils {
	...
	public static void setPreSetSysOut() {
		setOut("[" + IFormatter.FULL_DATE_TIME + "] " + "[" + IFormatter.THREAD_NAME + "/" + IFormatter.COLOR
				+ IFormatter.LEVEL + IFormatter.RESET + "] " + IFormatter.COLOR + IFormatter.MESSAGE
				+ IFormatter.RESET);
	}

	public static void setPreSetSysErr() {
		setErr("[" + IFormatter.FULL_DATE_TIME + "] " + "[" + IFormatter.THREAD_NAME + "/" + IFormatter.COLOR
				+ IFormatter.LEVEL + IFormatter.RESET + "] " + IFormatter.COLOR + IFormatter.MESSAGE
				+ IFormatter.RESET);
	}
	...
	public static void setLogAllLevel(String format, boolean formatAfterLineSeperator) {
		...
		g.setFormatter(
				new IFormatter(format, formatAfterLineSeperator).setFromLevel(Level.ALL).setToLevel(Level.CONFIG));
